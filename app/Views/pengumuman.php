        ...
        <div class="col-12 col-md-10">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title" style="text-transform: uppercase; font-size: 30px;">PENGUMUMAN UMUM</h4>
                </div>
...
<?php endif; ?>
    ...
    <a href="<?= base_url('home/hapus_pengumuman_umum/'.$okei->id_pengumuman) ?>">
...
function loadTambahPengumumanForm() {
    fetch('<?= base_url('home/t_pengumuman') ?>')
...
function loadEditPengumumanForm(id_pengumuman) {
    ...
    fetch('<?= base_url('home/e_pengumuman') ?>/' + id_pengumuman)
        .then(response => response.text())
        .then(data => {
            document.querySelector('.section').style.display = 'none';
            document.getElementById('dynamicContent').innerHTML = data;
